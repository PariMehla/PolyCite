...
- **Finding #3 (synonym-blind scoring): fixed, and validated on real data.**
  Cohere Embed cosine similarity cleanly separates the 3 known-correct-
  synonym cases from 3 known-genuinely-wrong cases: RIGHT similarity range
  `[0.563, 0.583]`, WRONG range `[0.486, 0.526]` — no overlap, suggested
  threshold ≈0.545. This validates embedding-similarity as a real
  supplement to literal-recall scoring (not yet wired into `scoring.py`'s
  default path — that's a deliberate next step, not done here).
...
tests/                   116 tests, all offline/mocked
