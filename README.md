...
- **Finding #3 (synonym-blind scoring): fixed, validated, and wired up to
  ...
  default (v1 stays judge-free by design, per "Engineering constraints"
  above; this is an explicit supplement, run separately):
  ```
...
These are the non-negotiable rules the codebase is built around — mainly
about not accidentally blowing through a free Cohere trial key:

- **1,000 Cohere calls/month, total, across every endpoint.** Every call
  goes through `polycite/cohere_client.py`, which (1) checks a persistent
  call counter in `cache/call_count.json` and refuses to exceed the
  budget, (2) caches every response on disk keyed by
  `sha256(endpoint + model + params + input)` so re-running anything that
  didn't change costs zero new calls, (3) rate-limits per endpoint (chat
  20/min, embed 100/min, rerank 10/min) with exponential backoff on 429.
- **Before any batch job**, `estimate_and_confirm()` prints the number of
  calls it's about to make and requires explicit confirmation
  (`POLYCITE_AUTO_CONFIRM=1` env var, or an interactive `y`) before
  spending a single call.
- **Never call Cohere in tests** — every test mocks `CohereClient` via
  `FakeTransport` (`polycite/testing.py`); this is how `make test` runs
  120 tests with $0 spend and no network.
- Batch embed calls at the max texts/call (96); use
  `input_type="search_document"` for passages, `"search_query"` for
  queries; temperature 0 everywhere; log the model name and version
  string on every result row.
- **Failure attribution priority**, checked in this order so each failure
  gets exactly one cause: retrieval miss > rerank miss > reading error >
  citation error > language error > hallucinated confidence.

Code style: small functions, type hints, a pytest test for every metric
function. No comments that restate what the code does — only ones that
explain a non-obvious *why* (e.g. CJK tokenization, FLORES/MIRACL style
mismatch).
